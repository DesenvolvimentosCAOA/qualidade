<cfinvoke method="inicializando" component="cf.ini.index">
    ...
    <cfquery name="consulta_nconformidades_pdi_n0" datasource="#BANCOSINC#">
        WITH CONSULTA AS (
            SELECT PROBLEMA, PECA, ESTACAO, COUNT(*) AS TOTAL_POR_DEFEITO
            FROM INTCOLDFUSION.SISTEMA_QUALIDADE_PDI_SAIDA
        ...
        WITH PrioritizedStatus AS (
            SELECT 
                ...
                ROW_NUMBER() OVER (PARTITION BY VIN 
                                ORDER BY 
                                CASE 
                                    WHEN STATUS = 'REPARO CAOA' THEN 1
                                    WHEN STATUS = 'LIBERADO' THEN 2
                                    WHEN STATUS = 'REPARO PDI' THEN 3
                                    ELSE 4
                                END) AS row_num
...
<html lang="pt-BR">
    <head>
        ...
        <title>PDI Indicador Geral-2º turno</title>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-sm w-100" style="font-size: 12px";>
                            ...
                            <tbody class="table-group-divider">
                                <cfoutput query="consulta_nconformidades_pdi">
    ...
    function drawChart() {
        ...
        chart.draw(data, options);
            ...
            <meta http-equiv="refresh" content="40,URL=pdi_indicadores_2.cfm">
    </body>
</html>
