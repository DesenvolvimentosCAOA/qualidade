...
<head>
    ...
    <title>Upload de Check List</title>
    <style>
        body {
...
<body>
    ...
    <cfscript>
        ...
        directoryPath = "#raizpasta#/qualidade/arquivo_checklist/";
        ...
        htmlTable &= "<tr><th>VIN</th><th>CAIXA</th><th>Ação</th></tr>";
        ...
        for (folder in folderList) {
            ...
            query.setSQL("SELECT VIN, CAIXA FROM ferramenta_rastreio WHERE VIN = :vin");
            query.addParam(name="vin", value=folderName, cfsqltype="cf_sql_varchar");
            result = query.execute().getResult();

            if (result.recordCount > 0) {
                htmlTable &= "<tr>";
                htmlTable &= "<td>" & result.VIN & "</td>";
                
                // Verifica se CAIXA está vazio ou nulo e coloca "Falta Lançamento"
                caixaValue = (len(trim(result.CAIXA)) == 0 or isNull(result.CAIXA)) ? "Falta Lançamento" : result.CAIXA;
                
                htmlTable &= "<td>" & caixaValue & "</td>";
                htmlTable &= "<td><a href='downloadFolder.cfm?folder=" & folderName & "'>Baixar Pasta</a> | <br><br><a href='deleteFolder.cfm?folder=" & folderName & "' onclick='return confirm('Tem certeza que deseja excluir esta pasta?')'>Excluir Pasta</a></td>";
                htmlTable &= "</tr>";
            }
