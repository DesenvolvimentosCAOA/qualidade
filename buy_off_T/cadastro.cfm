...
<cfquery name="inserir" datasource = "#BANCOSINC#">
    ...
    select max (user_id)+1, '#LCase(CADASTRO_USUARIO)#', '#cadastro_SENHA#', '#cadastro_NIVEL_ACESSO#', '#UCase(cadastro_NOME)#', SYSDATE-1 ,'#cadastro_SETOR#', '#cadastro_resp#' from intcoldfusion.reparo_fa_users
</cfquery>

<!-- Redireciona o usuário para a página de confirmação -->
<cflocation url="/qualidade/buy_off_t/fa_selecionar_buy_off.cfm" addtoken="no">
